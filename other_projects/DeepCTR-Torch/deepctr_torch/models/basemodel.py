...
class BaseModel(nn.Module):
    def __init__(self, linear_feature_columns, dnn_feature_columns, l2_reg_linear=1e-5, l2_reg_embedding=1e-5,
                 init_std=0.0001, seed=1024, task='binary', device='cpu', gpus=None,adt_config={}):
                ...
                with tqdm(enumerate(train_loader), disable=verbose != 1) as t:
                    for _, (x_train, y_train) in t:
                        ...
                        if self.adt_type=='fgm':#使用fgm对抗训练方式
                            ...
                            y_pred = model(x).squeeze()
                            loss_adv =loss_func(y_pred, y.squeeze(), reduction='sum')
                            ...
                            pgd.backup_grad()
                            # 对抗训练
                            for t_i in range(self.adt_k):
                                pgd.attack(epsilon=self.adt_epsilon,alpha=self.adt_alpha,is_first_attack=(t_i==0)) # 在embedding上添加对抗扰动, first attack时备份param.data
                                if t_i != self.adt_k - 1:
                                    optim.zero_grad()
                                else:
                                    pgd.restore_grad()
                            ...
                            # 对抗训练
                            for t_i in range(self.adt_k):
                                freeat.attack(is_first_attack=(t_i==0)) # 在embedding上添加对抗扰动, first attack时备份param.data
                                optim.zero_grad()
                            ...
                            # 对抗训练
                            for t_i in range(self.adt_k):
                                freelb.attack(is_first_attack=(t_i==0))  # 在embedding上添加对抗扰动, first attack时备份param.data
                                # self._optimizer.zero_grad()
                                y_pred = model(x).squeeze()
                                loss_adv =loss_func(y_pred, y.squeeze(), reduction='sum') / self.adt_k
                                loss_adv.backward() # 反向传播，并在正常的grad基础上，累加对抗训练的梯度
